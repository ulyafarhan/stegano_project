<template>
  <div class="text-white max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 sm:mt-16 mb-12">
    ...
    <div class="bg-slate-900 p-4 sm:p-6 lg:p-8 rounded-2xl shadow-2xl border border-slate-800">
      <form @submit.prevent="handleDecode">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          
          <div class="lg:col-span-7 space-y-4 sm:space-y-6">
            <div class="bg-indigo-500/10 border border-indigo-500/20 p-3 sm:p-4 rounded-xl mb-4 sm:mb-6">
              <div class="flex items-start gap-2 sm:gap-3">
                <i class="pi pi-info-circle text-indigo-400 text-lg sm:text-xl mt-0.5"></i>
          ...
          <div class="lg:col-span-5">
            <div class="bg-slate-800/50 p-4 sm:p-6 rounded-xl border border-slate-700 sticky top-4 sm:top-6">
              <h3 class="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-indigo-400 flex items-center gap-2">
                <i class="pi pi-key"></i> Kunci Keamanan
              </h3>
              
              <div class="space-y-4 sm:space-y-6">
                <div>
                  <label class="font-medium block mb-1 sm:mb-2 text-slate-300 text-sm sm:text-base">Masukkan Kunci (1-255)</label>
                  <InputNumber
                    v-model="key"
                    :min="1"
                    :max="255"
                    ...
                    class="w-full"
                    inputClass="text-center bg-slate-900 border-slate-600 text-white font-mono text-sm sm:text-base"
                  >
                    <template #incrementbuttonicon><span class="pi pi-plus" /></template>
                    <template #decrementbuttonicon><span class="pi pi-minus" /></template>
                  </InputNumber>
                  <small class="text-xs sm:text-sm text-slate-500 mt-1 sm:mt-2 block leading-tight">
                ...
                <div class="pt-3 sm:pt-4 border-t border-slate-700">
                  <Button
                    type="submit"
                    ...
                    :loading="isLoading"
                    :disabled="!isValid"
                    class="w-full p-button-lg bg-emerald-600 hover:bg-emerald-500 border-none shadow-lg shadow-emerald-900/20 transition-all transform hover:-translate-y-0.5 text-sm sm:text-base"
                  />
                </div>
              </div>
            </div>
